...
def parse_game(target_match_ids):
    ...
    target_match_ids = upcoming_games
    ...
    for game in data_list:
        if 'AA' in list(game.keys())[0]:
            ...
            if id_match in target_match_ids:
                date_timestamp = game.get("AD")
                date = datetime.fromtimestamp(int(date_timestamp)) if date_timestamp else None
    ...
    close_db(db_connection)  # Закрыть соединение с базой данных
